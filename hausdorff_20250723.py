...
# def poisson_disk_sample_mesh(input_path, target_points=35000):
#     ms = pymeshlab.MeshSet()
#     ms.load_new_mesh(input_path)
#     ms.generate_sampling_poisson_disk(samplenum=target_points, subsample=True)
#     return ms.current_mesh().vertex_matrix()

# def compute_hausdorff_metrics(ref_mesh_path, target_mesh_path, max_dist_threshold):
#     ref_sampled = poisson_disk_sample_mesh(ref_mesh_path)
#     tgt_sampled = poisson_disk_sample_mesh(target_mesh_path)

#     kdtree = cKDTree(ref_sampled)
#     hausdorff_distances, _ = kdtree.query(tgt_sampled)
...
#         "Reference Vertex Count": len(ref_sampled)
#     }

#     return hausdorff_distances, tgt_sampled, metrics
...
# def save_colored_mesh_ply(target_mesh_path, sampled_points, out_path, face_distances, max_dist_threshold):
#     mesh = trimesh.load_mesh(target_mesh_path, process=False)
...
#     colored_mesh.export(out_path)
#     print(f"✅ Saved heatmap mesh: {os.path.basename(out_path)}")
# def save_colored_pointcloud_ply(sampled_points, hausdorff_distances, out_path, max_dist_threshold):
#     bins = generate_scaled_bins(min_dist_threshold, max_dist_threshold)
#     color_map = generate_scaled_colors().astype(np.uint8)
#     bin_indices = np.clip(np.digitize(hausdorff_distances, bins, right=False), 0, len(color_map) - 1)
#     vertex_colors = color_map[bin_indices]
...
#     except:
#         print("⚠️ Convex hull failed. Saving as point cloud only.")
#         faces = []

#     colored_mesh = trimesh.Trimesh(vertices=sampled_points, faces=faces, vertex_colors=vertex_colors, process=False)
#     colored_mesh.export(out_path)
#     print(f"✅ Saved convex hull mesh from point cloud with vertex colors: {os.path.basename(out_path)}")
...
# def process_mesh_folder(mesh_folder, output_folder, output_csv, max_dist_threshold, mode="vertex"):
#     print(f"🟢 Processing folder: {mesh_folder} [mode={mode}]")
#     os.makedirs(output_folder, exist_ok=True)
#     mesh_files = sorted([f for f in os.listdir(mesh_folder) if f.lower().endswith(('.ply', '.stl', '.obj'))])
#     mesh_paths = {f: os.path.join(mesh_folder, f) for f in mesh_files}
#     all_metrics = []
...
#             ref_path, tgt_path = mesh_paths[ref_name], mesh_paths[tgt_name]
#             print(f"🔹 {ref_name} vs {tgt_name}")
#             hausdorff_distances, tgt_sampled, metrics = compute_hausdorff_metrics(ref_path, tgt_path, max_dist_threshold)
#             all_metrics.append(metrics)
...
#             if mode == "vertex":
#                 save_colored_pointcloud_ply(tgt_sampled, hausdorff_distances, out_path, max_dist_threshold)
#             else:
#                 tgt_mesh = trimesh.load_mesh(tgt_path, process=False)
#                 face_centroids = tgt_mesh.triangles_center
#                 kdtree = cKDTree(tgt_sampled)
#                 face_dists, _ = kdtree.query(face_centroids)
#                 save_colored_mesh_ply(tgt_path, tgt_sampled, out_path, face_dists, max_dist_threshold)
...
if __name__ == "__main__":
    mesh_folder = "/mnt/c/Users/klay.luke.PSYDUCK/Desktop/July 2025 results/meshlab/MHT/All meshes/clipped"
    ...
    image_matrix_file_path = f"image_matrix_{min_dist_threshold}mm_to_{max_dist_threshold}mm.png"
    #"image_matrix_biglabels.png"
    image_matrix_path = os.path.join(screenshot_folder, image_matrix_file_path)
