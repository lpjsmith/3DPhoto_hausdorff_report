...
def compute_hausdorff_metrics(ref_mesh_path, target_mesh_path, max_dist_threshold):
    ...
    rms_distance = np.sqrt(np.mean(hausdorff_distances ** 2))
    percentile_95 = np.percentile(hausdorff_distances, 95)
    under_max_dist_count = np.sum(hausdorff_distances < max_dist_threshold)
    ...
    metrics = {
        ...
        "RMS Distance (mm)": rms_distance,
        "95th Percentile Distance (mm)": percentile_95,
        f"Points < {max_dist_threshold}mm": under_max_dist_count,
    }
...
def save_metrics_to_csv(metrics_list, output_csv):
    ...
    # 🔹 Insert an empty first column to shift all data to the right
    df.insert(0, " ", None)
    ...
    # 🔹 Create a row for mean values (shifted right)
    mean_row = pd.DataFrame([{**{" ": "Mean Values"}, **mean_values}])  # Convert to DataFrame

    # 🔹 Concatenate DataFrames instead of `append()`
    df = pd.concat([df, blank_row, mean_row], ignore_index=True)
...
#     bottom_margin = -100  # 🔹 Corrected to properly position images from the bottom
#     text_offset = 20

#     page_width = num_meshes * cell_size
...
#         c.drawString(left_margin - text_offset, bottom_margin + (num_meshes - row) * cell_size + (cell_size / 2), short_name)
...
def generate_matrix_pdf(mesh_files, heatmap_images, output_pdf):
    ...
    cell_size = 80
    #if num_meshes > 10:
     #   cell_size = max(60, 900 // (num_meshes + 1))

    left_margin = 50  # Space for row labels
    bottom_margin = -50  # Set to 0 to avoid shifting content down
    text_offset = 20

    # Adjust page width and height *after* cell size adjustment
    page_width = (num_meshes + 2) * cell_size + left_margin 
    page_height = (num_meshes + 1)  * cell_size + text_offset  # Ensure square shape
    ...
    # Draw column headers
    for col, mesh_name in enumerate(mesh_files):
        short_name = mesh_name[:7]
        #c.drawString(left_margin + (col + 1) * cell_size, bottom_margin + (num_meshes + 1) * cell_size - text_offset, short_name)
        # Draw column headers (move them UP by adding `cell_size`)
        for col, mesh_name in enumerate(mesh_files):
            short_name = mesh_name[:7]
            c.drawString(
                left_margin + (col + 1) * cell_size,  # X position
                bottom_margin + (num_meshes + 1) * cell_size + (cell_size / 2),  # Y position (Raised)
                short_name
            )

    # Draw row headers & heatmap matrix
    for row, ref_mesh in enumerate(mesh_files):
        short_name = ref_mesh[:7]
        c.drawString(left_margin - text_offset, bottom_margin + (num_meshes - row) * cell_size + (cell_size / 2), short_name)
...
def generate_matrix_pdf_with_metrics(mesh_files, heatmap_images, metrics_csv, output_pdf):
    """Generates a PDF with a heatmap matrix and a table of metrics below it."""
    page_width, page_height = landscape(letter)
    extended_height = page_height * 2.5  # Make page 150% taller
    c = canvas.Canvas(output_pdf, pagesize=(page_width, extended_height))
    c.setFont("Helvetica", 10)

    cell_size = 80
    left_margin = 50
    bottom_margin = 0 #extended_height - (len(mesh_files) + 5) * cell_size  # Adjust for better spacing
    text_offset = 20
    ...
    # Draw row headers and heatmap images
    for row, ref_mesh in enumerate(mesh_files):
        short_name = ref_mesh[:7]
        ...
        for col, target_mesh in enumerate(mesh_files):
            ...
            if img_path and os.path.exists(img_path):
                ...
                print(f"⚠️ Missing heatmap for {ref_mesh} vs {target_mesh}: {img_path}")

    # Load metrics CSV
    df = pd.read_csv(metrics_csv)
    df.fillna("", inplace=True)
    df = df.round(3)  # Limit all numerical values to 3 decimal places
    df = df.applymap(lambda x: f"{x:.3f}"[:7] if isinstance(x, (int, float)) else str(x)[:7])  # Ensure numerical formatting and string truncation
    ...
    # Position the table below the heatmap matrix
    #table.wrapOn(c, 0, 0)
    #table.drawOn(c, 50, bottom_margin - 200)  # Move table down below the heatmap matrix
...
def compute_hausdorff_heatmap(ref_mesh_path, target_mesh_path, hausdorff_distances, max_dist_threshold):
    """Applies a heatmap to the target mesh using precomputed Hausdorff distances."""
    ...
    # 🔹 Generate dynamic bins based on `max_dist_threshold`
    #max_distance = 5.0
    color_bins = generate_scaled_bins(max_dist_threshold)
    ...
    for i, d in enumerate(face_distances):
        for j in range(len(color_bins) - 1):
            if color_bins[j] <= d < color_bins[j + 1]:
                ...
                break
        if d >= max_dist_threshold:
            face_colors[i] = [255, 0, 0, 255]  # Ensure anything ≥ {max_dist_threshold}mm is full red
...
def process_mesh_folder(mesh_folder, output_folder, output_pdf, output_csv, max_dist_threshold):
    """Runs the full pipeline: Computes Hausdorff metrics once, then generates heatmaps and a matrix PDF."""
    print(f"🟢 Processing meshes in folder: {mesh_folder}")
    os.makedirs(output_folder, exist_ok=True)

    mesh_files = sorted([f for f in os.listdir(mesh_folder) if f.lower().endswith(('.stl', '.obj', '.ply'))])
    mesh_paths = {f: os.path.join(mesh_folder, f) for f in mesh_files}

    all_metrics = []
    hausdorff_results = {}  # Store computed distances to reuse them

    # Compute Hausdorff Metrics for each pair (ONLY ONCE)
    for ref_mesh_name, target_mesh_name in itertools.product(mesh_files, repeat=2):
        ...
        print(f"🔹 Computing Hausdorff metrics for: {ref_mesh_name} vs {target_mesh_name}")

        # Compute distances once and store them
        hausdorff_distances, metrics = compute_hausdorff_metrics(ref_mesh_path, target_mesh_path, max_dist_threshold)
        all_metrics.append(metrics)
        hausdorff_results[(ref_mesh_name, target_mesh_name)] = hausdorff_distances  # Store distances

    # Save Metrics to CSV
    save_metrics_to_csv(all_metrics, output_csv)

    # 🔹 Generate heatmaps using stored distances
    for (ref_mesh_name, target_mesh_name), hausdorff_distances in hausdorff_results.items():
        ref_mesh_path = mesh_paths[ref_mesh_name]
        target_mesh_path = mesh_paths[target_mesh_name]

        print(f"🎨 Generating heatmap for: {ref_mesh_name} vs {target_mesh_name}")

        # Use the precomputed distances to create the heatmap
        mesh = compute_hausdorff_heatmap(ref_mesh_path, target_mesh_path, hausdorff_distances, max_dist_threshold)
    ...
    #generate_matrix_pdf_with_metrics(mesh_files, heatmap_images, output_csv, output_pdf)
...
if __name__ == "__main__":
    ...
    process_mesh_folder(mesh_folder, output_folder, output_pdf, output_csv, max_dist_threshold)
